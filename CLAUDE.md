...
- A running PostgreSQL (14+) with `DB/schema.sql` applied (creates 11 tables — including `comentario` from Sprint 3 — 3 enums, `set_atualizado_em` triggers; uses `pgcrypto` for `gen_random_uuid()`). **On an existing DB, never re-run `schema.sql`** — it starts with `DROP TABLE … CASCADE` and wipes all data. To pick up schema changes after a pull, apply the additive scripts in `DB/migrations/*.sql` instead (e.g. `001_add_comentario.sql`, idempotent).
...
### Admin mode
Admin status = a row in the `admin` table (1:1 with `usuario`) — there is no role column. `lib/admin.ts` exposes `isAdmin`, `findAdminIdByUsuarioId`, and the guard `requireAdmin(req)` → `{ ok: true, usuarioId, adminId } | { ok: false, status: 401 | 403 }` (401 = no session, 403 = logged-in non-admin). Admin endpoints live under `/api/admin/`, call `requireAdmin` first, and use the returned `adminId` for `bot.criado_por_id` / `puzzle.adicionado_por_id`. `users` is read-only (`GET` list). `puzzles` and `bots` are full CRUD: `index.ts` handles `GET` (list) + `POST` (create), and `[id].ts` handles `GET` (detail) + `PATCH` (edit) + `DELETE`. `isAdmin` is included in the `/api/users/me` and `/api/auth/login` responses (`MeResponse` in `apiClient.ts`). Client pages under `routes/administracao/` guard with the `useAdmin` hook (redirects 401→login, non-admin→profile). There is **no app-level promotion**: make an admin by running `DB/admin_seed.sql` against the DB. (A dev-only helper `POST /api/_internal/test/promote-admin` exists for E2E setup — it returns `401` outside development.)

### Comentários
Players comment on puzzles (Sprint 3). Endpoints under `/api/comentarios/`: `GET ?puzzleId=<lichessId>` lists a puzzle's comments (anonymous allowed), `POST` creates one (requires session), and `[id].ts` handles `PATCH` (edit) + `DELETE` (both owner-only). Repository is `lib/comentarios.ts`; the UI lives in `components/ui/ComentariosSection.tsx`. The `comentario` table keys off `puzzle_lichess_id` (TEXT, **no FK** to the local `puzzle` catalog, since players comment on puzzles fetched straight from Lichess) and is scoped by `jogador_id` (resolved from the session's `usuarioId`, same indirection as other jogador-scoped repos).
